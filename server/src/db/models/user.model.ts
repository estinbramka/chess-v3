...
export const create = async (user: User, id: string) => {
    ...
    try {
        const res = await db.query(
            `INSERT INTO "user"(name, email, id) VALUES($1, $2, $3) RETURNING id, name, email, wins, losses, draws`,
            [user.name, user.email || null, id]
        );
