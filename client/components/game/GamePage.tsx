...
import type { FormEvent, JSXElementConstructor, KeyboardEvent, PromiseLikeOfReactNode, ReactElement, ReactNode, ReactPortal } from "react";
...
export default function GamePage({ initialLobby, token }: { initialLobby: Game, token: string | undefined }) {
    ...
    useEffect(() => {
        if (
            ...
            !lobby.white ||
            !lobby.black ||
            (lobby.white.id !== user?.uid && lobby.black.id !== user?.uid)
        )
            return;
    ...
    useEffect(() => {
        ...
        syncSide(user, undefined, lobby, { updateLobby });

        initSocket(user, socket, lobby, {
            updateLobby,
            addMessage,
            updateCustomSquares,
            makeMove,
        ...
        try {
            ...
            if (result) {
                ...
                if (lobby.actualGame.inCheck()) {
                    const kingPos = lobby.actualGame.board().reduce((acc: any, row: any[], index: number) => {
                ...
                {(!lobby.white?.id || !lobby.black?.id) && (
                    <div className="absolute bottom-0 right-0 top-0 z-10 flex h-full w-full items-center justify-center bg-black bg-opacity-70">
                        <div className="bg-base-200 flex w-full items-center justify-center gap-4 px-2 py-4">
                            ...
                            {user?.uid !== lobby.white?.id && user?.uid !== lobby.black?.id && (
                                <button
            ...
            <div className="flex max-w-lg flex-1 flex-col items-center justify-center gap-4">
                ...
                <div className="relative h-60 w-full min-w-fit">
                    {(lobby.endReason ||
                        (lobby.pgn &&
                            lobby.white &&
                            user?.uid === lobby.white?.id &&
                            lobby.black &&
                            !lobby.black?.connected) ||
                        (lobby.pgn &&
                            lobby.black &&
                            user?.uid === lobby.black?.id &&
                            lobby.white &&
